 ...
 */
const main = async () => {
    const { api, specName, safeXcmVersion } = await (0, asset_transfer_api_1.constructApiPromise)('wss://kusama-asset-hub-rpc.polkadot.io');
    const assetApi = new asset_transfer_api_1.AssetTransferApi(api, specName, safeXcmVersion);
    let callInfo;
    try {
        callInfo = await assetApi.createTransferTransaction('2011', // Note: the Parchain ID matches the MultiLocations 'Parachain' ID, making this a teleportAssets
...
//# sourceMappingURL=systemToParaTeleportForeignAssets.js.map